 ...
 * reach a quorum.
 */
contract LegacyOptimisticIsm is IMultisigIsm, Ownable {
    // ============ Libraries ============
    ...
    mapping(uint32 => mapping(bytes32 => rootInfo)) infoAboutRoots;
    ...
    constructor(uint256 _window) Ownable() {
        window = _window;
    }
        ...
        returns (bool)
    {
        ...
        require(
            infoAboutRoots[_message.origin()][
    ...
    function commitRoot(
        ...
        require(validatorSet[domain].at(0) == signer);
        infoAboutRoots[domain][root].published = true;
        infoAboutRoots[domain][root].startingTime = block.timestamp;
    }

    function challengeRoot(bytes32 root, uint32 domain) external {
        require(
            infoAboutRoots[domain][root].published,
            "Root not published yet"
        );
        require(
            infoAboutRoots[domain][root].startingTime + window >
                block.timestamp,
